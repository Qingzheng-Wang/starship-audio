"""
Audio-focused app for starship - a client designed to fetch YouTube audio at scale.
Optimized for LAION-DISCO-12M and other audio datasets.
"""
...
flags.DEFINE_string("input", None, help="Input JSON file with audio download tasks")
...
flags.DEFINE_string("output_folder", "audio", help="The output folder to use within the GCP bucket for audio files.")
...
flags.DEFINE_string("instance_type", default="n1-highmem-2", help="The instance type for the server to use.")
flags.DEFINE_string("input_data_type", default="json", help="The type of input data to use.")
flags.DEFINE_bool("save_original_audio", True, help="Save original audio format without conversion")
...
def get_exit_handler(compute, create_ops: Dict[str, List[Any]], run_name: str) -> Callable[[], None]:
    """Create exit handler to clean up GCP instances on termination"""
    def exit_handler():
...
def _discover_running_instances(compute: Any, project: str, zones: List[str]) -> None:
    """Check if any starship instances are already running"""
    for zone in zones:
...
def _load_data_from_file(filepath: str, data_type: str) -> List[Dict[str, Any]]:
    """Load audio download tasks from input file"""
        ...
        if data_type == "file_list":
            ...
            for idx, entry in enumerate(data):
                if "song_id" not in entry:
                    logging.warning(f"Entry {idx} missing 'song_id', generating from index")
                    entry["song_id"] = f"audio_{idx:08d}"
                if "url" not in entry:
                    # Try to generate from song_id
                    if "song_id" in entry:
                        entry["url"] = f"https://www.youtube.com/watch?v={entry['song_id']}"
                    else:
                        raise ValueError(f"Entry {idx} missing both 'url' and 'song_id'")
            return data
        raise ValueError(f"Unknown input data type: {data_type}")


def _upload_code_to_gcp(storage_client: Any, bucket: str, audio_data: List[Dict[str, Any]]) -> None:
    """Upload server and worker scripts to GCP bucket"""
    # Upload the scripts and audio data to the GCP bucket
    upload_file_to_gcp(storage_client, bucket, "server.py", local_relpath(__file__, "servers/server.py"))
    upload_file_to_gcp(storage_client, bucket, "worker_audio_only.py", local_relpath(__file__, "workers/worker_audio_only.py"))
    storage_client.get_bucket(bucket).blob("videos.json").upload_from_string(json.dumps(audio_data))
    logging.info(f"Uploaded {len(audio_data)} audio download tasks to GCP bucket")
...
def _start_worker(
    ...
    """Start an audio worker instance"""
    try:
        worker_name = f"starship-{run_name}-wrk-{worker_index}"
        logging.info("Creating audio worker %s in zone %s", worker_name, zone)
        return create_instance(
            ...
            worker_name,
            bucket=bucket,
            startup_script_file=local_relpath(__file__, "workers/worker_audio_only.sh"),
...
def _cleanup_instances(compute: Any, project: str, zones: str, run_name: str) -> None:
    """Clean up all starship instances"""
...
def main(*unused_argv) -> None:
    ...
    audio_data = _load_data_from_file(FLAGS.input, FLAGS.input_data_type)
    logging.info(f"Loaded {len(audio_data)} audio download tasks")

    # Upload data to GCP bucket so the server can use it
    _upload_code_to_gcp(storage_client, FLAGS.bucket, audio_data)
    ...
    logging.info(
        """
        Audio download instances created.
        It will take some time for the instances to complete work.
        Check this URL: http://storage.googleapis.com/{}/audio/ for status.
        """.format(
            FLAGS.bucket
        )
    )
